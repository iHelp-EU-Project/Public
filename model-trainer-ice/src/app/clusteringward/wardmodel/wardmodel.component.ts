...
@Component({
  selector: 'app-wardmodel',
  templateUrl: './wardmodel.component.html',
  styleUrls: ['./wardmodel.component.css']
})
export class WardmodelComponent implements OnInit, AfterViewInit {
  ...
  async ngOnInit() {
    ...
    this.reactiveForm = this.builder.group({
      affinity: [null, Validators.required],
      cluster_k: [null, Validators.required],
      computeFullTree: [null, Validators.required],
      link: [null, Validators.required],
      classColumn: [null, Validators.required],
      computeDistances: [null, Validators.required],
  ...
  saveColumnsIDstring(id): string {
    ...
    // this.columnsId  = this.columnsId.substring(2);

    while (columnsId.charAt(0) === ',' || columnsId.charAt(0) === ' ') {
      columnsId = columnsId.substring(1);
    }
    console.log(columnsId);
    return columnsId;
  }
  ...
  runningScriptsMLKmeansEnd(): void {
    ...
    setTimeout(() => {
      ...
      try {
        // tslint:disable-next-line: max-line-length
        this.http.get(`/scripts/ScriptsWard/${this.modelName}/${this.currentUser.username}/${this.correlationTrainName}/${this.clusterK}/${this.choseDeleteColumnsString}/${this.classColumn}/${this.columnsPredict}/${this.getNameClases()}/${this.affinity}/${this.computeFullTree}/${this.computeDistances}/${this.link}/${this.valuesNan}/${this.advancedModel}`, { responseType: 'text' }).subscribe(data => {
