...
export class UploadFileComponent implements AfterViewInit, OnInit {
  ...
  runningScriptsML(): void {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        this.http.get(`/scripts/scriptsml/${this.modelName}/${this.uploadedFilesTrain[0].name}/${this.uploadedFilesTest[0].name}/${this.currentUser.username}`, { responseType: 'text' }).subscribe(data => {
  ...
  // Serealize datasets for view in graphics
