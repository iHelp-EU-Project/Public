...
import logging
from fastapi import FastAPI, Form, Request, Depends, Security, HTTPException, status, Response
from fastapi.responses import PlainTextResponse, HTMLResponse, FileResponse
from fastapi.staticfiles import StaticFiles
from fastapi.templating import Jinja2Templates
from fastapi_login import LoginManager
from fastapi.security import OAuth2PasswordRequestForm, OAuth2PasswordBearer, OAuth2PasswordRequestForm, HTTPBearer, OAuth2AuthorizationCodeBearer 
...
idp = FastAPIKeycloak(
    ...
    client_id="flask",
    client_secret="YFQnPw6hBsD5Z8StJqyDJb8P4hJdV57n",
    admin_client_secret="M6g7zuwTlE0lYVW0MrVpN4StmJ5Cr6Rx",
    realm="analyticsproduct",
    callback_uri="http://localhost:5002/callback"
)
idp.add_swagger_config(app)
...
@app.get('/ScriptsBirch/{modelName}/{userName}/{fileNameData}/{clusterK}/{choseDeleteColumnsString}/{classColumn}/{columnsPredict}/{nameClases}/{threshold}/{branching_factor}/{compute_labels}/{valuesNan}')
async def getModelBirch(modelName,userName,fileNameData,clusterK,choseDeleteColumnsString,classColumn,columnsPredict,nameClases,threshold,branching_factor,compute_labels,valuesNan):
    today =  datetime.date.today()
    dateToday = str(today.year) + str('{:02d}'.format(today.month)) + str('{:02d}'.format(today.day))
    sem.acquire()
    if platform == "linux" or platform == "linux2":
        os.system("python3.7  clusteringBirch/birch-script.py " + modelName +"."+ dateToday  + " " + userName + " " + fileNameData + " " + clusterK + " " + choseDeleteColumnsString + " " + classColumn + " " + columnsPredict + " " + nameClases + " " + threshold + " " + branching_factor + " " + compute_labels + " " + valuesNan + " > output.txt")
    else:
        os.system("python clusteringBirch/birch-script.py " + modelName +"."+ dateToday + " " + userName + " " +  fileNameData + " " + clusterK + " " + choseDeleteColumnsString + " " + classColumn + " " + columnsPredict + " " + nameClases + " " + threshold + " " + branching_factor + " " + compute_labels + " " + valuesNan +" > output.txt")
    sem.release() 
...
if __name__ == "__main__":
    uvicorn.run("initServerFastApi:app",host='0.0.0.0',port=5002)
