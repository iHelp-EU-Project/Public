...
def main(params):
    ...
    try:
        ...
        #Remove old files
        if(os.path.isfile(csvPath + "data_matrix.csv")):
                os.remove(csvPath + "data_matrix.csv")
                        
        if(os.path.isfile(csvPath + "data_prediction.csv")):
                os.remove(csvPath + "data_prediction.csv")
                
        if(os.path.isfile(csvPath + "data_regresion1.csv")):
                os.remove(csvPath + "data_regresion1.csv")
                
        if(os.path.isfile(csvPath + "dataGLMBinoMatizConfusionPandas.csv")):
                os.remove(csvPath + "dataGLMBinoMatizConfusionPandas.csv")
        ...
        loadingJSON(params['userName'], 20, 'Building training data', False);
        time.sleep(2.0)
        ...
        loadingJSON(params['userName'], 30, 'Building dataset', False);
        time.sleep(2.0)
        ...
        loadingJSON(params['userName'], 40, 'Building data chooseen users', False);
        time.sleep(2.0)    
        #Select data propierties choose users    
        work_final = work[work[params['onrearId']] == directionID ]
    
        loadingJSON(params['userName'], 60, 'Building the graphics', False);
        time.sleep(2.0);
        ...
        h2o.export_file(work_delete_onrearId, path = csvPath + "data_regresion1.csv" ,force=True)
        
        print(work_delete_onrearId)
    
        loadingJSON(params['userName'], 80, 'Saving the graphics', False);
        time.sleep(2.0);
