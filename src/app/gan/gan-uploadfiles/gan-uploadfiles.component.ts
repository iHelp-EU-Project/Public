...
@Component({
  selector: 'app-gan-uploadfiles',
  templateUrl: './gan-uploadfiles.component.html',
  styleUrls: ['./gan-uploadfiles.component.css']
})
export class GanUploadfilesComponent implements OnInit {
 ...
 selectedFiles: FileList;
  ...
  normalTrain = true;
  activeNormalTrainTOnormalTest = false;

  normalTest = false;
  activeNormalTestTOnormalValid = false;

  normalValid = false;
  activeNormalValidTObadTrain = false;

  badTrain = false;
  activeBadTrainTObadTest = false;



  badTest = false;
  activeBadTestTObadValid = false;

  badValid = false;
  activeBadValidTOGAN = false;
...
// }
  ...
  //         this.fileInfos = this.userService.getFiles(this.currentUser, 'pepe45', '/');
  //       }
  //     },
  //     err => {
  //       this.progress = 0;
...
runningScriptsTensorGAN(): void {
      ...
      try {
        this.http.get(`/scripts/ScriptsGanXRay/${this.modelName}/${this.currentUser.username}/${this.learningRate}/${this.channels}/${this.batchSize}/${this.latent_dim}/${this.cols}/${this.rows}/${this.epochs}/`, {responseType: 'text'})
          .subscribe(data => {
            this.informationWorkflow = data as string;

            this.userService.compressFilesGAN(this.currentUser, this.modelnameDate , this.currentUser.username).subscribe(
              event => {
                this.showToaster('Create zip file with all workspace!')

              },
              err => {
                this.showToasterError('Could not create zip file with all workspace!');
              });
  ...
  uploadImagesNormalTrain(): void{
    ...
    this.activeNormalTrainTOnormalTest = true;
  ...
  normalTrain_TO_NormalTest(): void{

     this.normalTrain = false;
     this.normalTest = true;
  }



  uploadImagesNormalTest(): void{

    // Actualizar la propiedad "progress" con el porcentaje de carga
    this.progress = 0;
    this.imagesViewMenu = true;
    this.endRadioCheck = true;
    let formData = new FormData();
    ...
    Array.from(this.selectedFiles).forEach(file => {
      ...
      this.userService.uploadFilesGAN(this.currentUser, this.modelnameDate, 'test/normal/', this.currentFile).subscribe(
        event => {
          if (event.type === HttpEventType.UploadProgress) {
            this.progress = Math.round(100 * event.loaded / event.total);
          } else if (event instanceof HttpResponse) {
            ...
            // download name files for view in frond end
            this.fileInfos = new Observable<any>();
            this.fileInfos = this.userService.getFiles(this.currentUser,  this.modelName + '.' + this.pipeDate, 'test/normal/');
          }
        },
        err => {
          this.progress = 0;
    ...
    this.activeNormalTestTOnormalValid = true;
    // Reset the "progress" property to 0
    this.progress = 0;
  }


  normalTest_TO_NormalValid(): void{
  ...
  uploadImagesNormalValid(): void {
    ...
    Array.from(this.selectedFiles).forEach(file => {
      ...
      this.userService.uploadFilesGAN(this.currentUser, this.modelnameDate, 'valid/normal/', this.currentFile).subscribe(
        event => {
          if (event.type === HttpEventType.UploadProgress) {
            this.progress = Math.round(100 * event.loaded / event.total);
          } else if (event instanceof HttpResponse) {
            ...
            // download name files for view in frond end
            this.fileInfos = new Observable<any>();
            this.fileInfos = this.userService.getFiles(this.currentUser, this.modelName + '.' + this.pipeDate, 'valid/normal/');
          }
        },
        err => {
          this.progress = 0;
    ...
    this.activeNormalValidTObadTrain = true;
    // Reset the "progress" property to 0
    this.progress = 0;

  }

  normalValid_TO_BadTrain(): void{
  ...
  uploadImagesBadTrain(): void {
    ...
    Array.from(this.selectedFiles).forEach(file => {
      ...
      this.userService.uploadFilesGAN(this.currentUser, this.modelnameDate, 'train/bad/', this.currentFile).subscribe(
        event => {
          if (event.type === HttpEventType.UploadProgress) {
            this.progress = Math.round(100 * event.loaded / event.total);
          } else if (event instanceof HttpResponse) {
            ...
            // download name files for view in frond end
            this.fileInfos = new Observable<any>();
            this.fileInfos = this.userService.getFiles(this.currentUser, this.modelName + '.' + this.pipeDate, 'train/bad/');
          }
        },
        err => {
          this.progress = 0;
    ...
    this.activeBadTrainTObadTest = true;
    // Reset the "progress" property to 0
    this.progress = 0;
  }


  badTrain_TO_BadTest(): void{
  ...
  uploadImagesBadTest(): void{
    ...
    Array.from(this.selectedFiles).forEach(file => {
      ...
      this.userService.uploadFilesGAN(this.currentUser, this.modelnameDate, 'test/bad/', this.currentFile).subscribe(
        event => {
          if (event.type === HttpEventType.UploadProgress) {
            this.progress = Math.round(100 * event.loaded / event.total);
          } else if (event instanceof HttpResponse) {
            ...
            // download name files for view in frond end
            this.fileInfos = new Observable<any>();
            this.fileInfos = this.userService.getFiles(this.currentUser, this.modelName + '.' + this.pipeDate, 'test/bad/');
          }
        },
        err => {
          this.progress = 0;
    ...
    this.activeBadTestTObadValid = true;
    // Reset the "progress" property to 0
    this.progress = 0;
  }


  badTest_TO_BadValid(): void{
 ...
 uploadImagesbadValid(): void {

    // Set the "imagesViewMenu" property to true
    this.imagesViewMenu = true;

    // Set the "endRadioCheck" property to true
    this.endRadioCheck = true;
    ...
    Array.from(this.selectedFiles).forEach(file => {
      ...
      // Initialize the "progress" property to 0
      this.progress = 0;

      // Create a new instance of FormData
      formData = new FormData();

      // Store the current file in the "currentFile" property
      this.currentFile = file;

      // Add the current file to the FormData object
      formData.append('file', file);

      // Call the user service's "uploadFilesGAN" method to upload the file
      this.userService.uploadFilesGAN(this.currentUser, this.modelnameDate, 'valid/bad/', this.currentFile).subscribe(
        event => {
          // If the event is an upload progress
          if (event.type === HttpEventType.UploadProgress) {
            // Update the "progress" property with the percentage of progress
            this.progress = Math.round(100 * event.loaded / event.total);
          }
          // If the event is an HTTP response
          else if (event instanceof HttpResponse) {
            // Store the message in the "message" property
            this.message = event.body.message;
            // Get the names of the downloaded files to view in the front-end
            this.fileInfos = new Observable<any>();
            this.fileInfos = this.userService.getFiles(this.currentUser, this.modelName + '.' + this.pipeDate, 'valid/bad/');
          }
        },
        // In case of error
        err => {
          ...
          this.message = 'Could not upload the file!';
          // Reset the "currentFile" property
          this.currentFile = undefined;
        });
      // Reset the "selectedFiles" property
      this.selectedFiles = undefined;
    });

    // Set the "activeBadValidTOGAN" property to true
    this.activeBadValidTOGAN = true;
    // Reset the "progress" property to 0
    this.progress = 0;
  }


  badValid_TO_ScriptGAN(): void{
